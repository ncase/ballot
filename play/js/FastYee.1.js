...
// const has a local scope inside any braces, so since we are doing this if(runscript) thing, we need to have const defined outside the block
var gpu,d,cubeVoter,cubeCandidate,minCube,maxCube,doFnorm,doScores,doTally,findWinner,superKernel,superKernel2,superKernelCandidate,superKernelVoter,cube3
...
d = gpu.createKernel( function(xv, yv, xf, yf) {
...
cubeVoter = gpu.createKernel( function(xv,yv,xc,yc,xvcenter,yvcenter,vg,votergrouptomove,pixelsize,liy,lix){
...
cubeCandidate = gpu.createKernel( function(xv,yv,xc,yc,candidatetomove,pixelsize,liy,lix){
...
minCube = gpu.createKernel( function(d) {
	var iv,civ,n,d1
	iv = this.thread.x
...
maxCube = gpu.createKernel( function(d) {
...
doFnorm = gpu.createKernel(function(m,n) {
...
doScores = gpu.createKernel(function(d,m,n,fnorm, minscore, maxscore) {
...
doTally = gpu.createKernel(function(scores) {
	var ci,sum
	ci = this.thread.x
	sum = 0
	for (var v = 0; v < lv; v++) {
...
findWinner = gpu.createKernel(function(tally) {
	var i,ci,m,t1,mi
	i = this.thread.x
	m = -1
	mi = -1
...
if (!onlylast) {
superKernel = gpu.combineKernels(d,minCube,maxCube,doFnorm,doScores,doTally,findWinner, function(xv,yv,xf,yf) {
...
superKernel2 = gpu.combineKernels(minCube,maxCube,doFnorm,doScores,doTally,findWinner, function(cube2) {
...
superKernelCandidate = gpu.combineKernels(cubeCandidate,minCube,maxCube,doFnorm,doScores,doTally,findWinner, function(xv,yv,xc,yc,candidatetomove,pixelsize,liy,lix) {
		var cube,min,max,fnorm,scores,tally,winner
...
superKernelVoter = gpu.combineKernels(cubeVoter,minCube,maxCube,doFnorm,doScores,doTally,findWinner, function(xv,yv,xc,yc,xvcenter,yvcenter,vg,votergrouptomove,pixelsize,liy,lix) {
		var cube,min,max,fnorm,scores,tally,winner
...
function fastyee(xc,yc,xf,yf,xv,yv,vg,xvcenter,yvcenter,movethisidx,whichtypetomove,WIDTH,HEIGHT,density) {
	pixelsize = density
	lix = WIDTH / pixelsize // called density
	liy = HEIGHT / pixelsize
